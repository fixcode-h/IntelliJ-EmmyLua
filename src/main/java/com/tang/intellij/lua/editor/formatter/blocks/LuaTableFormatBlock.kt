...
class LuaTableFormatBlock(psi: PsiElement,
    ...
    // 缓存文档对象以提高性能
    private val document by lazy { psi.containingFile.viewProvider.document }
    ...
    override fun buildChild(child: PsiElement, indent: Indent?): LuaScriptBlock {
        ...
        // 特殊处理注释对齐（使用工具类）
        if (child is PsiComment && ctx.luaSettings.ALIGN_LINE_COMMENTS) {
            val alignmentConfig = LuaCommentAlignmentUtil.createCommentBlockAlignment(child, document, ctx)
            if (alignmentConfig.symbolAlignment != null) {
                return LuaCommentBlock(child, childWrap, alignmentConfig.symbolAlignment, childIndent, ctx)
            }
        }
        
        val block = createBlock(child, childIndent, childAlignment, childWrap)
